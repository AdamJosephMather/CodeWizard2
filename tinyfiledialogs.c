	...
	wchar_t const * aDefaultPath)
{
	...
	wchar_t * lRetval = NULL;
	HRESULT hr;
	BOOL comInit = FALSE;
	BOOL triedModernDialog = FALSE;
	BOOL userCancelledModernDialog = FALSE;
	BOOL modernDialogUnavailable = FALSE;

	if (aTitle && !wcscmp(aTitle, L"tinyfd_query")) {
		wcscpy_s((wchar_t*)tinyfd_response, MAX_PATH, L"windows_wchar");
		return (wchar_t*)1;
	}
	...
	if (SUCCEEDED(hr)) {
		...
		if (SUCCEEDED(hr) && pfd) {
			triedModernDialog = TRUE;

			DWORD opts;
			if (SUCCEEDED(pfd->lpVtbl->GetOptions(pfd, &opts))) {
				pfd->lpVtbl->SetOptions(
					pfd,
			...
			if (aDefaultPath && wcslen(aDefaultPath)) {
				...
				if (SUCCEEDED(SHCreateItemFromParsingName(
					...
					(void**)&psiInit)))
				{
					pfd->lpVtbl->SetFolder(pfd, psiInit);
					psiInit->lpVtbl->Release(psiInit);
				}
			}
			...
			if (hr == HRESULT_FROM_WIN32(ERROR_CANCELLED)) {
				userCancelledModernDialog = TRUE;
			}
			else if (SUCCEEDED(hr)) {
				IShellItem *psiRes = NULL;

				if (SUCCEEDED(pfd->lpVtbl->GetResult(pfd, &psiRes))) {
					PWSTR pszFolder = NULL;

					if (SUCCEEDED(psiRes->lpVtbl->GetDisplayName(
						psiRes,
						SIGDN_FILESYSPATH,
						&pszFolder)))
					{
						wcscpy_s(lBuff, MAX_PATH, pszFolder);
						CoTaskMemFree(pszFolder);
						lRetval = lBuff;
					}

					psiRes->lpVtbl->Release(psiRes);
				}
			}
	...
	/*
		Only fall back if the modern dialog was unavailable or genuinely failed
		before being shown.

		Do NOT fall back after user cancellation.
	*/
	if (!lRetval && !userCancelledModernDialog && !triedModernDialog) {
		BROWSEINFOW bInfo = { 0 };
		LPITEMIDLIST pidl;
		...
		if (pidl) {
			if (SHGetPathFromIDListW(pidl, lBuff)) {
				lRetval = lBuff;
			}
