	...
	wchar_t const * aDefaultPath) /* NULL or L"" */
{
	...
	/* respond to tinyfd_query */
	if (aTitle && !wcscmp(aTitle, L"tinyfd_query")) {
		/* windows_wchar means we have a wide version */
		wcscpy_s((wchar_t*)tinyfd_response, MAX_PATH, L"windows_wchar");
		return (wchar_t*)1;
	}

	/* Try Vista-style IFileOpenDialog with FOS_PICKFOLDERS */
	...
	/* Fall back to legacy SHBrowseForFolder if COM approach failed */
	if (!lRetval) {
		BROWSEINFOW bInfo = { 0 };
		LPITEMIDLIST pidl;
